...
public class Tree {
    ...
    public boolean isNamePyramid() {
        // проверяем левого ребёнка: его имя не должно быть короче текущего
        if (left != null) {
            if (left.name.length() < name.length()) {
                return false;
            }
        ...
        // проверяем правого ребёнка: его имя не должно быть короче текущего
        if (right != null) {
            if (right.name.length() < name.length()) {
                return false;
            }
